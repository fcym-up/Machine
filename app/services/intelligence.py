...
"""IntelligenceService 鈥?妯″紡妫€娴嬩笌椋庨櫓鍒嗘瀽銆?

detect_patterns锛氶鐜囧垎鏋愩€侀珮宄版椂娈垫娴嬨€乀op 绫诲瀷/鏉ユ簮銆?
risk_score锛氬熀浜庤鍒欑殑椋庨櫓璇勪及锛堥珮棰戙€侀敊璇巼锛夈€?
trend_analysis锛氭寜鏃ヨ秼鍔垮垽鏂紙澧為暱/绋冲畾/涓嬮檷锛夈€?
"""
...
class IntelligenceService:
    ...
    def detect_patterns(self, days: int = 7) -> dict:
        ...
        patterns.append({
            "type": "peak_hour",
            "detail": f"Most active at hour {peak_hour[0]}:00",
            "count": peak_hour[1],
        })
        logger.info(f"Detected {len(patterns)} patterns from {total} events")
        return {
            "total_events": total,
            "period_days": days,
            "patterns": patterns,
    ...
    def risk_score(self) -> dict:
        ...
        if total == 0:
            return {"overall_risk": 0.0, "factors": [], "total_events": 0}
        recent = self.db.query(Event).filter(
            Event.created_at >= datetime.now(timezone.utc).replace(tzinfo=None) - timedelta(hours=1)
        ).count()
        factors = []
        risk = 0.0
        if recent > 20:
            ...
            factors.append({
                ...
                "detail": f"{recent} events in last hour",
                "contribution": 30.0,
            })
        ...
        if error_count > 5:
            ...
            factors.append({
                "factor": "error_rate",
                "detail": f"{error_count} error events detected",
                "contribution": 20.0,
            })
        risk = min(risk, 100.0)
        return {
    ...
    def trend_analysis(self, days: int = 7) -> dict:
        ...
        daily_data = [{"day": str(row[0]), "count": row[1]} for row in daily_counts]
